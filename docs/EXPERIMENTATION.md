...
| 2026-04-22 | Exp 19: blueprint grounding — first pass (verbatim-return). Matched blueprint returned as-is with goal-context header prepended. Fast (2 s) but generic demo with a sticker. User feedback: "our purpose is to deliver a customized plan." | Exp 19 |
| 2026-04-22 | Exp 20: blueprint-as-grounding (blueprint content in LLM system prompt). First attempt produced 4-cell stub — LLM drifting into prose outside JSON array format. Root cause: single JSON-only directive at bottom of prompt forgotten after 20k-token blueprint reference. | Exp 20 |
| 2026-04-22 | Exp 20 fixes: trimmed reference to code+substantive markdown (58k→25k chars); JSON-only directive at BOTH top and bottom with STOP guidance; explicit 30–60 cell target. | Exp 20 |
| 2026-04-22 | Exp 21 runs across three prompts. Full review: `docs/test-results/exp21-blueprint-grounded-runs.md` | Exp 21 |
| 2026-04-22 | **Finding (21a, vague healthcare):** 31 cells, 7:15, **8.0 / 10**. Real `nemoguardrails` imports + Colang clinical flows with PHI entities + red-flag detection. Replaces Exp 17's Python-regex Guardrails fake. | Exp 21 |
| 2026-04-22 | **Finding (21b, detailed healthcare):** 40 cells, 7:15, **8.5 / 10 (session best)**. Real Epic FHIR stub with OAuth2 flow + RxNorm terminology (code 860975 = Metformin) + real Triton HTTP protocol + NIM off-prem fallback matching user's specific prompt. | Exp 21 |
| 2026-04-22 | **Finding (21c, detailed fraud):** 22 cells, 2:11, **7.5 / 10**. Real Triton config.pbtxt grammar (vs Exp 17/18's malformed dict-to-f-string) + real HMAC-SHA256 audit logging with SOX 7-year retention + Kafka stub. Correct modern `trtexec --onnx --saveEngine --int8` CLI (vs Exp 18's deprecated `builder.*` API). | Exp 21 |
| 2026-04-22 | **Finding (21c residual):** cell 9 has hard `SyntaxError` (`from pytorch_modelopt import torch.quantization as mtq` — invalid; garbled duplicate line). Plus persistent `rapids==24.06` pip hallucination across all three fraud runs (Exp 18 + Exp 21c). Class fix: add `rapids` to AST validator known-bad-packages list. | Exp 21 |
| 2026-04-22 | **Class-level confirmed:** blueprint grounding adds genuine customization (not a sticker). FHIR stub + RxNorm + Kafka stub + SOX retention came from GoalSpec signals, not the blueprint. Blueprint grounding fixes API-correctness classes from Exp 17/18 (real NeMo Guardrails API, real pbtxt grammar, current TRT CLI). Output ceiling scales with blueprint quality. | Exp 21 |
| 2026-04-22 | **Still open:** (a) `rapids==24.06` pip class — AST validator known-bad list. (b) Stage 2 Retriever drop on vague RAG prompts — blueprint retrieval at Stage 2 or typed catalog. (c) Cell-level syntax errors (21c cell 9) — self-heal loop on Brev would close deterministically. (d) `lib/blueprint-parameterizer.ts` retired dead code — remove on next cleanup. | Exp 21 |
